...
public:
    ...
    int whisperPort = 8090;
    bool voiceFreeMode = true;
    std::string voiceModelPath;
    int voiceLlmPort = 8095;
    float voiceVadThresh = 0.015f;
    int voiceSilenceMs = 900;
    int voicePttVk = 0x20; // VK_SPACE
    int voicePttMods = 0;
    std::string voiceFolder;
    bool voiceWakeEnabled = false;
    std::string voiceWakeWord = "Assistente";
    bool minimizeToTray = false;
    bool closeToTray = false;
    int voiceInputDevice = -1;
    int voiceOutputDevice = -1;
    bool voiceThinkingEnabled = false;
    std::string voiceStopWord;
    bool voiceWebSearchEnabled = false;
    int voiceTtsVoice = -1;
    ...
    void Load(const std::string& path = "config.json") {
        ...
        try {
            ...
            whisperModelPath = j.value("whisper_model_path", "");
            whisperPort = j.value("whisper_port", 8090);
            voiceFreeMode = j.value("voice_free_mode", true);
            voiceModelPath = j.value("voice_model_path", "");
            voiceLlmPort = j.value("voice_llm_port", 8095);
            voiceVadThresh = j.value("voice_vad_thresh", 0.015f);
            voiceSilenceMs = j.value("voice_silence_ms", 900);
            voicePttVk = j.value("voice_ptt_vk", 0x20);
            voicePttMods = j.value("voice_ptt_mods", 0);
            voiceFolder = j.value("voice_folder", "");
            voiceWakeEnabled = j.value("voice_wake_enabled", false);
            voiceWakeWord = j.value("voice_wake_word", "Assistente");
            minimizeToTray = j.value("minimize_to_tray", false);
            closeToTray = j.value("close_to_tray", false);
            voiceInputDevice = j.value("voice_input_device", -1);
            voiceOutputDevice = j.value("voice_output_device", -1);
            voiceThinkingEnabled = j.value("voice_thinking_enabled", false);
            voiceStopWord = j.value("voice_stop_word", "");
            voiceWebSearchEnabled = j.value("voice_web_search_enabled", false);
            voiceTtsVoice = j.value("voice_tts_voice", -1);
    ...
    void Save() {
        ...
        j["whisper_model_path"] = whisperModelPath;
        j["whisper_port"] = whisperPort;
        j["voice_free_mode"] = voiceFreeMode;
        j["voice_model_path"] = voiceModelPath;
        j["voice_llm_port"] = voiceLlmPort;
        j["voice_vad_thresh"] = voiceVadThresh;
        j["voice_silence_ms"] = voiceSilenceMs;
        j["voice_ptt_vk"] = voicePttVk;
        j["voice_ptt_mods"] = voicePttMods;
        j["voice_folder"] = voiceFolder;
        j["voice_wake_enabled"] = voiceWakeEnabled;
        j["voice_wake_word"] = voiceWakeWord;
        j["minimize_to_tray"] = minimizeToTray;
        j["close_to_tray"] = closeToTray;
        j["voice_input_device"] = voiceInputDevice;
        j["voice_output_device"] = voiceOutputDevice;
        j["voice_thinking_enabled"] = voiceThinkingEnabled;
        j["voice_stop_word"] = voiceStopWord;
        j["voice_web_search_enabled"] = voiceWebSearchEnabled;
        j["voice_tts_voice"] = voiceTtsVoice;
